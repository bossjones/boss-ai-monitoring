...
|---|---|
| [`boss-ai-monitoring-build-team.md`](boss-ai-monitoring-build-team.md) | **Current (rev 3, 2026-07-11).** The 7-pane run that *builds* what the spec describes — [`specs/boss-ai-monitoring/boss-ai-monitoring.html`](../specs/boss-ai-monitoring/boss-ai-monitoring.html) Phases 1–9, TDD red-first, entirely in place in this repo (no new GitHub repo, no push). Lead runs opus for coordination; all 6 workers (store, otlp, jsonl+langsmith, web, jobs, validator) run sonnet. Rev 2 adds a PREFLIGHT toolchain block (duckdb + langsmith CLIs now installed), the `bam serve` entrypoint owner, spec-drift fixes from a full gap analysis (dedupe key, fixture sets, edge cases), and a LangSmith read-back verification loop via the `langsmith` CLI with direnv-ambient auth. Rev 3 extracts per-pane spec briefs to [`specs/boss-ai-monitoring/briefs/`](../specs/boss-ai-monitoring/briefs/) — the HTML stays canonical for humans; agents read their scoped markdown brief instead. Rev 4 fixes the fresh-eyes audit findings: `request_id` schema column + extraction (the dedupe key existed only in prose), dual-port serving owned by `bam serve`, lead scaffolds all package/tests skeletons, hook-safe live-telemetry recipe, `bam config db-path` for GATE checks, and the fast-Docker-loop directive (iterate on `uv run`, compose up minimally). |
